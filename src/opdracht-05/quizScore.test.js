...
describe('Quiz Score', () => {
  ...
  test('isPassed geeft true bij 60% of hoger', () => {
    ...
    const passed = isPassed(quizAnswers);
    expect(passed).toBe(true); // Deze test faalt! Vervang met je eigen test
  });
  ...
  test('isPassed geeft false bij lager dan 60%', () => {
    ...
    const passed = isPassed(failedAnswers);
    expect(passed).toBe(false); // Deze test faalt! Vervang met je eigen test
  });
  ...
  test('getGrade geeft Excellent bij 90%+', () => {
    ...
    const grade = getGrade(excellentAnswers);
    expect(grade).toBe('Excellent'); // Deze test faalt! Vervang met je eigen test
  });

  test('getGrade geeft Goed bij 75-89%', () => {
    // TODO: Gebruik quizAnswers (80% goed)
    // TODO: Check of getGrade 'Goed' teruggeeft
    const grade = getGrade(quizAnswers);
    expect(grade).toBe('Goed'); // Deze test faalt! Vervang met je eigen test
  });
  ...
  test('countWrongAnswers telt foute antwoorden', () => {
    ...
    const wrongAnswers = countWrongAnswers(quizAnswers);
    expect(wrongAnswers).toBe(1); // Deze test faalt! Vervang met je eigen test
  });
